...
--xem thong tin benh nhan vua them  theo ten
go
create proc sp_ThongTinBenhNhanVuaThem  
as
begin
	select * from BenhNhan where Ma=(select Max(Ma) from BenhNhan)
end

--exec sp_ThongTinBenhNhanVuaThem 'Hao'
...
----1--xem thong tin BS trong 1 khoa
go
--drop proc sp_ThongTinBacSiTrongKhoa
create proc sp_ThongTinBacSiTrongKhoa @MaKhoa int
as
begin
	select bs.*,k.Ten
	from BacSi bs,Khoa k
	where MaKhoa=@MaKhoa and k.Ma=bs.Ma
end
...
--roi
---drop proc sp_DanhSachBacSiTrucTrongNgay
go
create proc sp_DanhSachBacSiTrucTrongNgay @ngay datetime
as
begin
	select lt.*,bs.Ten
	from LichTruc lt,BacSi bs
	where Ngay= @ngay and lt.MaBacSi=bs.Ma
end
...
--exec sp_DemSLBenhNhanDKTrongNgay '08/10/2009 2:57:55 PM'
go
...
--drop proc sp_DemSLBenhNhanDKTrongNgay
--exec sp_DemSLBenhNhanDKTrongNgay '2008-07-04'
...
-- ngay  7-8
--exec sp_LayThongTinTKMoiThem
go
create proc sp_LayThongTinTKMoiThem
as
begin
	select * from TaiKhoan where Ma= (select Max(Ma) from TaiKhoan)
end
--ngay 9-8
-- lay thong tin tu tai khoan
go
  --drop proc sp_ThongTinTK
create proc sp_ThongTinTK @Ten nvarchar(20),@MatKhau nvarchar(20)
as
begin
	 select tk.* from TaiKhoan tk where MatKhau=@MatKhau and tk.Ten=@Ten
end


go
--exec sp_MaBenhNhanNguoiLonKhiBietMaTaiKhoan 7
create proc sp_MaBenhNhanNguoiLonKhiBietMaTaiKhoan @MaTK int
as
begin
	select MaBenhNhan from NguoiLon where MaTaiKhoan=@MaTK
end
go

--exec sp_LayThongTinBangThamSoCoMa 1
 create proc sp_LayThongTinBangThamSoCoMa @MaTS int
as
begin
	select * from ThamSo where  Ma=@MaTS
end
-- lay ma phieu dang ky tu mabenh nhan va ngay
go
create proc sp_LayMaPhieuDangKyTuMaBenhNhan @MaBN int, @Ngay datetime
as
begin
 select * from PhieuDangKy where MaBenhNhan=@MaBN and Ngay=@Ngay
end
